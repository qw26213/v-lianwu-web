<template>
  <div class="app-container">
    ...
    <el-table :key="tableKey" v-loading="listLoading" :data="tableData" border fit highlight-current-row style="width: 100%;">
      ...
      <el-table-column label="审核意见" v-if="activeStatus==-1">
        <template slot-scope="{row}">
      ...
      <el-table-column label="操作" align="center" v-if="activeStatus==0">
        <template slot-scope="{row}">
          <el-button type="primary" size="mini" @click="detail(row)" v-permission="['C010301','C010302']">审核</el-button>
        </template>
      </el-table-column>
    </el-table>
...
<script>
import { listStoreAudit } from '@/api/user'
...
export default {
  ...
  methods: {
    ...
    getList() {
      ...
      listStoreAudit(this.listQuery).then(response => {
    ...
    detail(obj){
      this.$router.push('/storeAudit?id='+obj.id);
    },
  }
}
